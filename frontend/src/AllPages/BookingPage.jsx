...
const normalizeDoctor = (raw) => {
  ...
  const name = raw.d_name ?? raw.name ?? raw.doctor_name ?? raw.doctorName ?? null;
  const email = raw.d_email ?? raw.email ?? null;
  const specialization = raw.specialization ?? raw.speciality ?? raw.department ?? "Specialist";
...
const getPatientFromStorage = () => {
  ...
  const id =
    Number(localStorage.getItem("user_id")) || Number(user?.id) || Number(user?.user?.id) || Number(user?.patient?.id);
  const name =
...
const PaymentLogo = ({ method, onClick, disabled, isSelected }) => (
  <div
    ...
    <span className={`payment-logo-text ${method.toLowerCase()}`}>{method}</span>
  </div>
);
...
const Calendar = ({ selectedDate, setSelectedDate }) => {
  ...
  const renderHeader = () => (
    <div className='calendar-header d-flex justify-content-between align-items-center mb-3'>
      <button className='btn btn-outline-primary btn-sm' onClick={() => setCurrentMonth(subMonths(currentMonth, 1))}>
        &lt;
      </button>
      <span className='fw-semibold'>{format(currentMonth, "MMMM yyyy")}</span>
      <button className='btn btn-outline-primary btn-sm' onClick={() => setCurrentMonth(addMonths(currentMonth, 1))}>
        &gt;
      </button>
    </div>
  );
      ...
      <div className='calendar-grid d-flex flex-wrap'>
        {days.map((day) => {
          ...
          return (
            <div
              key={day.toISOString()}
              className={`day p-2 text-center border m-1 rounded ${!isSameMonth(day, monthStart) ? "text-muted" : ""} ${
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Redirect to DocAppointment page after booking
        navigate(`/appointments/${doctor.id}`); // Navigate to Doctor's appointments page
      } else {
  ...
  if (!doctor?.name || !(doctor?.id || doctor?.email)) {
    return (
      <div className='container text-center py-5'>
        ...
        <p className='text-muted'>Doctor info is missing. Please go back and select a doctor again.</p>
          ...
          <div className='card shadow-sm border-0 mb-4'>
            <div className='card-body p-4 d-flex align-items-center'>
              <div className='doctor-img-placeholder me-3'>
                <svg width='60' height='60' viewBox='0 0 100 100' fill='#e9ecef' aria-hidden='true'>
              ...
              <div>
                ...
                <p className='mb-0 text-muted'>Consultation Fee: {consultationFee} BDT</p>
              </div>
            </div>
          </div>
